...
export const MyRides = () => {
  ...
  return (
    <div className={styles.mainContainer}>
      ...
      {data.map((item: any) => (
        <Rides rideId={item.rideId} func={func} />
      ))}
      <NavLink to="/newride" className={styles.newRide}>
        <img src={plus} className={styles.newRide} />
      </NavLink>
...
const Chat = ({ rideId, setChat }) => {
  ...
  const sendMessage = async () => {
    const id = sessionStorage.getItem("user");
    try {
      const response = await axiosApi.post("/api/message/user", {
        message: myMessage,
        rideId: rideId,
        userId: id,
      });
